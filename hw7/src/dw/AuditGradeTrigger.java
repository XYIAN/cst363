...
private static final String selectCred_sql =
		  "select credits from course where course_id=?";
	
private static final String insert_sql =
      "insert into takes (id, course_id, sec_id, semester, year, grade) "+
      " values(?, ?, ?, ?, ?, ?)";
private static final String updateTakes_sql =
		  "update takes set id=?, course_id=?, sec_id=?, semester=?, year=?, grade=? "+
		 " where id=? and course_id=?";
//private static final String updateStudent_sql =
//		  "update student set tot_cred = ? where id = ?";
//
private static final String updateAddStudent_sql =
		  "update student set tot_cred += ? where id=?";
private static final String updateRemoveStudent_sql =
		  "update student set tot_cred -= ? where id=?";
private static final String delete_sql = "delete from takes where id=? and course_id=?";
...
//private static final SimpleDateFormat dateFormatter = new SimpleDateFormat("yyyy-MM-dd");
	...
	public void fire(Connection conn, Object[] oldRow, Object[] newRow) throws SQLException {
		...
		if("F".equals(newRow[GRADE])) {
				...
				int rowSize = delete_stmt.executeUpdate();
				...
				rowSize = insert_stmt.executeUpdate();
		}
		else if ("A".equals(newRow[GRADE]) || "B".equals(newRow[GRADE]) || "C".equals(newRow[GRADE]) || "D".equals(newRow[GRADE])) {
				// student passes course, insert row to audit table
				//String currentDate = dateFormatter.format(new java.util.Date());
				
				PreparedStatement updateStudent_stmt = conn.prepareStatement(updateAddStudent_sql);
				PreparedStatement selectCred_stmt = conn.prepareStatement(selectCred_sql);
				PreparedStatement insert_stmt = conn.prepareStatement(insert_sql);
     // sql column index starts at 1, not 0
				
				selectCred_stmt.setObject(1,newRow[COURSE_ID]);
				insert_stmt.setObject(1, newRow[ID]);
				insert_stmt.setObject(2, newRow[COURSE_ID]);
				insert_stmt.setObject(3, newRow[SEC_ID]);
				insert_stmt.setObject(4, newRow[SEMESTER]);
				...
				long num_rows = updateStudent_stmt.executeUpdate();
				System.out.println("gradeChecker insert num_rows = " + num_rows);  // for debug
			}
			else {
				// when student fails course, remove any audit rows for student, course
...
*/
	...
	public void init(Connection conn, String schema, String trigger, String table, boolean before, int type) throws SQLException {
		 ...
		 */
  // for debug
		System.out.println("gradeChecker init called.");
...
@Override
	public void close() throws SQLException  {
		System.out.println("gradeChecker close.");  // debug
	}
/*
...
*/
	...
	public void remove() throws SQLException {
		System.out.println("gradeChecker deleted.");
	}
}